...
$script:PackCatalog = @(
    ...
    # TEST35: extra local promo rotation. These deliberately reuse the proven
    # player-only pack generator and existing GOLD/SILVER/BRONZE display assets.
    [ordered]@{ id=17; name='Gold Players Pack';            level='gold';   coins=12500; points=$null; rares=1;  premium=$true; size=12; group='gold';   assetId=3 },
    [ordered]@{ id=18; name='Small Rare Gold Players Pack'; level='gold';   coins=30000; points=$null; rares=6;  premium=$true; size=6;  group='promo';  assetId=4 },
    [ordered]@{ id=19; name='Mega Gold Players Pack';       level='gold';   coins=55000; points=$null; rares=12; premium=$true; size=18; group='promo';  assetId=4 },
    [ordered]@{ id=20; name='Ultimate Players Pack';        level='gold';   coins=125000;points=$null; rares=30; premium=$true; size=30; group='promo';  assetId=4 },
    [ordered]@{ id=21; name='Small Rare Silver Players Pack';level='silver';coins=10000; points=$null; rares=6;  premium=$true; size=6;  group='silver'; assetId=2 },
    [ordered]@{ id=22; name='Small Rare Bronze Players Pack';level='bronze';coins=2500;  points=$null; rares=6;  premium=$true; size=6;  group='bronze'; assetId=1 },

    # TEST39: parser-safe mixed Gold promo packs. They use the already-proven
    # native non-player generator instead of inventing consumable resource ids.
    [ordered]@{ id=23; name='Jumbo Premium Gold Pack'; level='gold'; coins=15000; points=$null; rares=7;  premium=$true; size=24; playerCount=7;  group='promo'; assetId=4 },
    [ordered]@{ id=24; name='Mega Pack';               level='gold'; coins=35000; points=$null; rares=18; premium=$true; size=30; playerCount=12; group='promo'; assetId=4 },
    [ordered]@{ id=25; name='Rare Mega Pack';          level='gold'; coins=55000; points=$null; rares=30; premium=$true; size=30; playerCount=12; group='promo'; assetId=4 }
)
...
# TEST28: one coherent squad subsystem instead of the TEST21-27 one-off patches.
# Main squad (id 1) keeps using the proven v0.14.1 SQLite-backed globals.
# Extra squads are independent session-local records with their own name,
# formation, arithmetic and 23-slot item-id order.
$script:Squad28Extra = [ordered]@{}
$script:Squad28NextId = 2
$script:Squad28ActiveId = 1
# TEST33 protects the squad snapshot used to create an offline match.
$script:Match33ArmedSquadId = 0
$script:Match33ProtectedOrder = @()
$script:Match33ProtectedCount = 0
# TEST36: selectable club presentation items. Resource ids default to the
# proven TEST17 match assets; owned items can replace them through PUT /item/<id>.
$script:Active36HomeKitResourceId = 6300000
$script:Active36AwayKitResourceId = 6400000
$script:Active36StadiumResourceId = 6200015
$script:Active36BadgeResourceId = 0
$script:Test37MaxTradePile = 9999
$script:Test37MaxConsumables = 9999
...
function Get-Item36CatalogRow([object]$Item) {
    if($null -eq $Item -or $null -eq $Item.resourceId){return $null}
    return $script:ItemByResourceId[[string][long]$Item.resourceId]
}

function Get-Presentation37SavePath {
    $dir=Split-Path -Parent $SavePath
    if([string]::IsNullOrWhiteSpace($dir)){$dir=(Join-Path $rootDirectory 'save')}
    return (Join-Path $dir 'fut13-presentation.json')
}

function Persist-Presentation37State {
    try {
        $path=Get-Presentation37SavePath
        $dir=Split-Path -Parent $path
        New-Item -ItemType Directory -Force -Path $dir|Out-Null
        $doc=[ordered]@{
            version=1
            homeKitResourceId=[long]$script:Active36HomeKitResourceId
            awayKitResourceId=[long]$script:Active36AwayKitResourceId
            stadiumResourceId=[long]$script:Active36StadiumResourceId
            badgeResourceId=[long]$script:Active36BadgeResourceId
        }
        $tmp="$path.tmp"
        $doc|ConvertTo-Json -Compress|Set-Content -LiteralPath $tmp -Encoding UTF8
        Move-Item -LiteralPath $tmp -Destination $path -Force
        Write-Log ("TEST37 PRESENTATION SAVE home={0} away={1} stadium={2} badge={3}" -f $script:Active36HomeKitResourceId,$script:Active36AwayKitResourceId,$script:Active36StadiumResourceId,$script:Active36BadgeResourceId)
    } catch {
        Write-Log ("TEST37 PRESENTATION SAVE ERROR: {0}" -f $_.Exception.Message)
    }
}

function Load-Presentation37State {
    try {
        $path=Get-Presentation37SavePath
        if(-not (Test-Path -LiteralPath $path)){return}
        $doc=Get-Content -LiteralPath $path -Raw|ConvertFrom-Json
        foreach($spec in @(
            @{name='homeKitResourceId';kind='kit';cat=2;target='home'},
            @{name='awayKitResourceId';kind='kit';cat=3;target='away'},
            @{name='stadiumResourceId';kind='stadium';cat=4;target='stadium'},
            @{name='badgeResourceId';kind='badge';cat=1;target='badge'}
        )){
            $rid=[long]$doc.($spec.name)
            if($rid -le 0){continue}
            $row=$script:ItemByResourceId[[string]$rid]
            if($null -eq $row){continue}
            $kind=([string]$row.kind).ToLowerInvariant()
            if($kind -ne $spec.kind){continue}
            if($spec.kind -in @('kit','badge','stadium') -and [int]$row.category -ne [int]$spec.cat){continue}
            switch($spec.target){
                'home'    {$script:Active36HomeKitResourceId=$rid}
                'away'    {$script:Active36AwayKitResourceId=$rid}
                'stadium' {$script:Active36StadiumResourceId=$rid}
                'badge'   {$script:Active36BadgeResourceId=$rid}
            }
        }
        Write-Log ("TEST37 PRESENTATION LOAD home={0} away={1} stadium={2} badge={3}" -f $script:Active36HomeKitResourceId,$script:Active36AwayKitResourceId,$script:Active36StadiumResourceId,$script:Active36BadgeResourceId)
    } catch {
        Write-Log ("TEST37 PRESENTATION LOAD ERROR: {0}" -f $_.Exception.Message)
    }
}

function Set-Test37ItemState([object]$Item,[string]$State) {
    if($null -eq $Item){return}
    if($null -eq $Item.PSObject.Properties['itemState']){
        $Item|Add-Member -NotePropertyName itemState -NotePropertyValue $State -Force
    } else {
        $Item.itemState=$State
    }
}

function Move-Test37OwnedItemToClub([long]$ItemId,[object]$Item) {
    # S12 showed activated cosmetics later resurfacing in "items to deal with".
    # Activation is therefore also an ownership/pile transition: once accepted,
    # remove the selected item from unassigned/trade and keep the same item id in club.
    $script:PurchasedItems=@($script:PurchasedItems|Where-Object {[long]$_.id -ne $ItemId})
    $script:PurchasedDuplicatePairs=@($script:PurchasedDuplicatePairs|Where-Object {[long]$_.itemId -ne $ItemId})
    $script:TradePileItems=@($script:TradePileItems|Where-Object {[long]$_.id -ne $ItemId})
    if(-not @($script:ClubExtraItems|Where-Object {[long]$_.id -eq $ItemId}).Count){
        $script:ClubExtraItems += $Item
    }
}

function Deactivate-Test37Role([string]$Kind,[int]$Category,[long]$ExceptItemId) {
    foreach($candidate in @($script:ClubExtraItems)){
        if([long]$candidate.id -eq $ExceptItemId){continue}
        $row=Get-Item36CatalogRow $candidate
        if($null -eq $row){continue}
        if(([string]$row.kind).ToLowerInvariant() -eq $Kind -and [int]$row.category -eq $Category){
            Set-Test37ItemState $candidate 'inactive'
        }
    }
}

function Get-Test37ActiveAssetId([long]$ResourceId,[int]$Fallback) {
    $row=$script:ItemByResourceId[[string]$ResourceId]
    if($null -ne $row -and $null -ne $row.assetId){return [int]$row.assetId}
    return $Fallback
}

function Set-ActiveClubItem36([long]$ItemId,[string]$RequestBody) {
    $item=Get-OwnedPlayerByItemId ([int]$ItemId)
    if($null -eq $item){return '{"success":false}'}
    $row=Get-Item36CatalogRow $item
    if($null -eq $row){return '{"success":false}'}

    $state=''
    try{$doc=$RequestBody|ConvertFrom-Json;if($null -ne $doc.itemState){$state=([string]$doc.itemState).ToLowerInvariant()}}catch{}
    if($state -ne 'active'){return '{"success":false}'}

    $kind=([string]$row.kind).ToLowerInvariant()
    $category=[int]$row.category

    # Native FIFA13 catalogue categories resolve the role without guessing:
    # badge=1, home kit=2, away kit=3, stadium=4.
    if($kind -eq 'kit' -and $category -eq 2){
        Deactivate-Test37Role 'kit' 2 $ItemId
        $script:Active36HomeKitResourceId=[long]$item.resourceId
        $role='activeHomeKit'
    }elseif($kind -eq 'kit' -and $category -eq 3){
        Deactivate-Test37Role 'kit' 3 $ItemId
        $script:Active36AwayKitResourceId=[long]$item.resourceId
        $role='activeAwayKit'
    }elseif($kind -eq 'stadium'){
        Deactivate-Test37Role 'stadium' 4 $ItemId
        $script:Active36StadiumResourceId=[long]$item.resourceId
        $role='activeStadium'
    }elseif($kind -eq 'badge'){
        Deactivate-Test37Role 'badge' 1 $ItemId
        $script:Active36BadgeResourceId=[long]$item.resourceId
        $role='activeBadge'
    }else{
        Write-Log ("TEST37 ACTIVE rejected unsupported kind/category item={0} kind={1} category={2}" -f $ItemId,$kind,$category)
        return '{"success":false}'
    }

    Move-Test37OwnedItemToClub $ItemId $item
    Set-Test37ItemState $item $role
    Persist-SaveState
    Persist-Presentation37State

    Write-Log ("TEST37 ACTIVE CLUB ITEM role={0} item={1} resource={2} asset={3}" -f $role,$ItemId,$item.resourceId,$row.assetId)

    # Return the updated item itself inside the same itemData family used by
    # ordinary item moves, plus the direct fields accepted by the previous build.
    return ([ordered]@{
        id=[long]$ItemId
        itemId=[long]$ItemId
        resourceId=[long]$item.resourceId
        itemState=[string]$role
        success=$true
        itemData=@($item)
...
function Build-SquadRecord {
    ...
    $actives = @()
    foreach($spec in @(
        @{ resourceId=[long]$script:Active36HomeKitResourceId; state='activeHomeKit'; type='kit' },
        @{ resourceId=[long]$script:Active36AwayKitResourceId; state='activeAwayKit'; type='kit' },
        @{ resourceId=[long]$script:Active36StadiumResourceId; state='activeStadium'; type='stadium' },
        @{ resourceId=[long]$script:Active36BadgeResourceId; state='activeBadge'; type='badge' }
    )) {
        if([long]$spec.resourceId -le 0){continue}
        $owned = $null
        foreach($candidate in @($script:ClubExtraItems)) {
...
# ---------------------------------------------------------------------------
# TEST30: persistent multi-squad sidecar + authoritative active-squad metadata
# ---------------------------------------------------------------------------
function Get-Squad30SavePath {
    $dir=Split-Path -Parent $SavePath
    if([string]::IsNullOrWhiteSpace($dir)){$dir=(Join-Path $rootDirectory 'save')}
    return (Join-Path $dir 'fut13-multisquads.json')
}

function Get-Squad30ActiveName {
    if([int]$script:Squad28ActiveId -eq 1){return [string]$script:SquadName}
    $x=Get-Squad28Extra ([int]$script:Squad28ActiveId)
    if($null -ne $x){return [string]$x.squadName}
    return [string]$script:SquadName
}


function Get-Squad30ActiveFormation {
    $id=[int]$script:Squad28ActiveId
    if($id -eq 1){return [string]$script:SquadFormation}
    $rec=$script:Squad28Extra[[string]$id]
    if($null -ne $rec -and -not [string]::IsNullOrWhiteSpace([string]$rec.formation)){return [string]$rec.formation}
    return 'f442'
}

function Persist-Squad30State {
    try{
        $path=Get-Squad30SavePath
        $dir=Split-Path -Parent $path
        New-Item -ItemType Directory -Force -Path $dir|Out-Null
        $rows=@()
        foreach($key in @($script:Squad28Extra.Keys)){
            $st=$script:Squad28Extra[$key]
            $ord=@(0)*23
            for($i=0;$i -lt 23;$i++){
                if($i -lt @($st.order).Count){$ord[$i]=[int]$st.order[$i]}
            }
            $rows += ,[ordered]@{
                id=[int]$st.id
                squadName=[string]$st.squadName
                formation=[string]$st.formation
                chemistry=[int]$st.chemistry
                starRating=[int]$st.starRating
        ...
        $doc=[ordered]@{
            version=1
            activeSquadId=[int]$script:Squad28ActiveId
            nextSquadId=[int]$script:Squad28NextId
            squads=@($rows)
        }
        $tmp="$path.tmp"
        $doc|ConvertTo-Json -Depth 10 -Compress|Set-Content -LiteralPath $tmp -Encoding UTF8
        Move-Item -LiteralPath $tmp -Destination $path -Force
        Write-Log ("TEST30 PERSIST: squads={0} active={1} path={2}" -f $rows.Count,$script:Squad28ActiveId,$path)
    }catch{
        Write-Log ("TEST30 PERSIST ERROR: {0}" -f $_.Exception.Message)
    }
}

function Load-Squad30State {
    try{
        $path=Get-Squad30SavePath
        if(-not (Test-Path -LiteralPath $path)){
            Write-Log ("TEST30 LOAD: no persistent multi-squad file yet: {0}" -f $path)
            return
        }
        $doc=(Get-Content -LiteralPath $path -Raw|ConvertFrom-Json)
        $loaded=[ordered]@{}
        $maxId=1
        foreach($row in @($doc.squads)){
            $id=[int]$row.id
            if($id -le 1){continue}
            $name=[string]$row.squadName
            if([string]::IsNullOrWhiteSpace($name)){$name="Squad $id"}
            $formation=[string]$row.formation
            if([string]::IsNullOrWhiteSpace($formation)){$formation='f442'}
            $ord=@(0)*23
            for($i=0;$i -lt [Math]::Min(23,@($row.order).Count);$i++){
                $candidate=[int]$row.order[$i]
                # Keep only item IDs that still exist in the persisted club.
                if($candidate -gt 0 -and $null -ne (Get-OwnedPlayerByItemId $candidate)){$ord[$i]=$candidate}
            }
            $loaded[[string]$id]=[ordered]@{
                id=$id;squadName=$name;formation=$formation
                chemistry=[int]$row.chemistry;starRating=[int]$row.starRating;rating=[int]$row.rating
                order=@($ord)
            }
            if($id -gt $maxId){$maxId=$id}
        }
        $script:Squad28Extra=$loaded
        $script:Squad28NextId=[Math]::Max($maxId+1,[int]$doc.nextSquadId)
        if($script:Squad28NextId -lt 2){$script:Squad28NextId=2}
        $wanted=[int]$doc.activeSquadId
        if($wanted -eq 1 -or $script:Squad28Extra.Contains([string]$wanted)){$script:Squad28ActiveId=$wanted}else{$script:Squad28ActiveId=1}
        Write-Log ("TEST30 LOAD: squads={0} active={1} next={2} names={3}" -f $script:Squad28Extra.Count,$script:Squad28ActiveId,$script:Squad28NextId,(($script:Squad28Extra.Values|ForEach-Object{$_.squadName}) -join ' | '))
    }catch{
        $script:Squad28Extra=[ordered]@{};$script:Squad28NextId=2;$script:Squad28ActiveId=1
        Write-Log ("TEST30 LOAD ERROR: {0}" -f $_.Exception.Message)
    }
}

function Get-Squad28Extra([int] $SquadId) {
    $key=[string]$SquadId
    if($script:Squad28Extra.Contains($key)){ return $script:Squad28Extra[$key] }
    return $null
}

function Set-Squad28Active([int] $SquadId) {
    if($SquadId -eq 1){
        $script:Squad28ActiveId=1
        Write-Log "TEST30 ACTIVE -> 1"
        Persist-Squad30State
        return $true
    }
    if($null -ne (Get-Squad28Extra $SquadId)){
        $script:Squad28ActiveId=$SquadId
        Write-Log ("TEST30 ACTIVE -> {0}; name={1}" -f $SquadId,[string](Get-Squad28Extra $SquadId).squadName)
        Persist-Squad30State
        return $true
    }
    Write-Log ("TEST28 ACTIVE REJECT unknown squad {0}" -f $SquadId)
    return $false
}

function Build-Squad28Record([int] $SquadId) {
    if($SquadId -eq 1){
        $record=Build-SquadRecord
        $record['squadId']=1
        $record['name']=[string]$script:SquadName
        $record['active']=([int]$script:Squad28ActiveId -eq 1)
        $record['changed']=$false
        $record['valid']=$true
        $record['newsquad']=0
        if($null -ne $script:SquadRating){$record['rating']=[int]$script:SquadRating}
        return $record
    }

    $state=Get-Squad28Extra $SquadId
    if($null -eq $state){ return $null }

    # Critical TEST28 fix: only item IDs are stored. Every GET rebuilds FULL
    # ItemData through the exact same Get-OwnedPlayerByItemId path as squad 1.
    $players=@()
    for($i=0;$i -lt 23;$i++){
        $itemId=0
        if($i -lt @($state.order).Count){$itemId=[int]$state.order[$i]}
        $item=if($itemId -gt 0){Get-OwnedPlayerByItemId $itemId}else{$null}
        $players += [ordered]@{
            index=$i
            itemData=$(if($null -eq $item){[ordered]@{id=0}}else{$item})
        }
    ...
    # Re-use the already proven TEST17 active kit/stadium projection so an
    # extra squad can later enter the offline match engine without regressing
    # to the grey/void pitch.
    $presentation=Build-SquadRecord
    $actives=@()
    if($null -ne $presentation.actives){$actives=@($presentation.actives)}

    $record=[ordered]@{
        id=[int]$state.id
        squadId=[int]$state.id
        personaId=[long]$script:PersonaId
        squadName=[string]$state.squadName
        name=[string]$state.squadName
        formation=[string]$state.formation
        active=([int]$script:Squad28ActiveId -eq [int]$state.id)
        changed=$false
        captain=0
        chemistry=[int]$state.chemistry
        starRating=[int]$state.starRating
...
function ConvertTo-Squad28DetailJson([object] $Record) {
    if($null -eq $Record){return '{}'}
    # Main squad's proven parser contract is wrapper + duplicate bare fields.
    # TEST28 uses that SAME contract for every numbered squad.
    $root=New-Object System.Collections.Specialized.OrderedDictionary([System.StringComparer]::Ordinal)
    $root.Add('squad',$Record)
    foreach($key in $Record.Keys){$root.Add([string]$key,$Record[$key])}
    # TEST30: explicit authoritative selection metadata for the detail/header model.
    $root.Add('activeSquadId',[int]$script:Squad28ActiveId)
    $root.Add('activeSquadName',[string](Get-Squad30ActiveName))
    $root.Add('selectedSquadId',[int]$Record.id)
    $root.Add('selectedSquadName',[string]$Record.squadName)
    return ($root | ConvertTo-Json -Depth 12 -Compress)
}

function ConvertTo-Squad31SelectedDetailJson([object] $Record) {
    if($null -eq $Record){return '{}'}
    $selected=[ordered]@{}
    foreach($key in $Record.Keys){$selected[[string]$key]=$Record[$key]}
    $selected['activeSquadId']=[int]$script:Squad28ActiveId
    $selected['selectedSquadId']=[int]$Record.id
    return ($selected | ConvertTo-Json -Depth 12 -Compress)
}

function Build-Squad28ListDocument {
    $rows=@()
    ...
    $rows += [ordered]@{
        ...
        squadId=1
        personaId=[long]$script:PersonaId
        squadName=[string]$script:SquadName
        name=[string]$script:SquadName
        formation=[string]$script:SquadFormation
        active=([int]$script:Squad28ActiveId -eq 1)
        changed=$false
        chemistry=$(if($null -eq $script:SquadChemistry){0}else{[int]$script:SquadChemistry})
        starRating=$(if($null -eq $script:SquadStarRating){0}else{[int]$script:SquadStarRating})
        rating=$(if($null -eq $script:SquadRating){0}else{[int]$script:SquadRating})
        valid=$true
        newsquad=0
    }

    foreach($key in @($script:Squad28Extra.Keys)){
        $state=$script:Squad28Extra[$key]
        $rows += [ordered]@{
            id=[int]$state.id
            squadId=[int]$state.id
            personaId=[long]$script:PersonaId
    ...
    Write-Log ("TEST28 LIST: active={0} rows={1} names={2}" -f `
        $script:Squad28ActiveId,$rows.Count,(($rows | ForEach-Object {$_.squadName}) -join ' | '))

    return ([ordered]@{
        activeSquadId=[int]$script:Squad28ActiveId
        activeSquadName=[string](Get-Squad30ActiveName)
        selectedSquadId=[int]$script:Squad28ActiveId
        selectedSquadName=[string](Get-Squad30ActiveName)
        squad=@($rows)
    } | ConvertTo-Json -Depth 8 -Compress)
}

function New-Squad28([string] $JsonBody) {
    try{
        $doc=$JsonBody | ConvertFrom-Json
        $id=[int]$script:Squad28NextId
        $script:Squad28NextId++
        ...
        $script:Squad28Extra[[string]$id]=$state
        $script:Squad28ActiveId=$id

        $record=Build-Squad28Record $id
        Write-Log ("TEST30 CREATE: id={0} name={1} formation={2}" -f $id,$name,$formation)
        Persist-Squad30State

        # Keep TEST24's proven create contract: POST /squad returns the created
        # full record itself. The immediate numbered GET then uses wrapper+bare.
        return ($record | ConvertTo-Json -Depth 12 -Compress)
    }catch{
        Write-Log ("TEST28 CREATE ERROR: {0}" -f $_.Exception.Message)
        return '{}'
    }
}


function Get-Squad33Order([int]$SquadId) {
    if($SquadId -eq 1){ return @($script:SquadOrder) }
    $state=Get-Squad28Extra $SquadId
    if($null -ne $state){ return @($state.order) }
    return @()
}
function Get-Squad33PlayerCountFromDocument([object]$Doc) {
    $count=0
    if($null -ne $Doc -and $null -ne $Doc.players){
        foreach($slot in @($Doc.players)){
            if($null -ne $slot -and $null -ne $slot.itemData -and [int64]$slot.itemData.id -gt 0){$count++}
        }
    }
    return $count
}
function Arm-Match33Squad([int]$SquadId) {
    if($SquadId -le 0){$SquadId=[int]$script:Squad28ActiveId}
    [void](Set-Squad28Active $SquadId)
    $order=@(Get-Squad33Order $SquadId)
    $script:Match33ArmedSquadId=$SquadId
    $script:Match33ProtectedOrder=@($order)
    $script:Match33ProtectedCount=@($order|Where-Object{[int64]$_ -gt 0}).Count
    Write-Log ("TEST33 MATCH SNAPSHOT squad={0} cards={1}" -f $SquadId,$script:Match33ProtectedCount)
    Persist-Squad30State
}
function Test-Squad33StaleMatchWrite([int]$SquadId,[object]$Doc) {
    if([int]$script:Match33ArmedSquadId -ne $SquadId){return $false}
    if($null -eq $Doc -or $null -eq $Doc.players){return $false}
    $incoming=Get-Squad33PlayerCountFromDocument $Doc
    if($incoming -lt [int]$script:Match33ProtectedCount){
        Write-Log ("TEST33 IGNORE STALE POST-MATCH SQUAD PUT id={0}: incomingCards={1} protectedCards={2}" -f $SquadId,$incoming,$script:Match33ProtectedCount)
        return $true
    }
    return $false
}

function Test-Squad38TransientZeroReset([int]$SquadId,[object]$Doc) {
    if($null -eq $Doc -or $null -eq $Doc.players){return $false}
    $incoming=Get-Squad33PlayerCountFromDocument $Doc
    if($incoming -ne 0){return $false}

    $saved=@(Get-Squad33Order $SquadId)
    $savedCount=@($saved|Where-Object{[int64]$_ -gt 0}).Count
    if($savedCount -le 0){return $false}

    $chem=if($null -ne $Doc.chemistry){[int]$Doc.chemistry}else{-1}
    $star=if($null -ne $Doc.starRating){[int]$Doc.starRating}else{-1}
    $rating=if($null -ne $Doc.rating){[int]$Doc.rating}else{-1}

    # S13 exact false-reset signature: a previously populated squad is followed
    # by a complete 23-slot id:0 snapshot with all arithmetic reset to zero.
    # This is a client transition/cache clear, not an intentional squad save.
    if($chem -eq 0 -and $star -eq 0 -and $rating -eq 0){
        Write-Log ("TEST38 IGNORE TRANSIENT ZERO SQUAD RESET id={0}: savedCards={1}" -f $SquadId,$savedCount)
        return $true
    }
    return $false
}

function Save-Squad28Extra([int] $SquadId,[string] $JsonBody) {
    $state=Get-Squad28Extra $SquadId
    if($null -eq $state -or [string]::IsNullOrWhiteSpace($JsonBody)){return '{}'}

    try{
        $doc=$JsonBody | ConvertFrom-Json
        if(Test-Squad33StaleMatchWrite $SquadId $doc -or Test-Squad38TransientZeroReset $SquadId $doc){
            return (ConvertTo-Squad28DetailJson (Build-Squad28Record $SquadId))
        }

        # squadName in FIFA 13's normal PUT is demonstrably stale/blank across
        # switches. The CREATE name owns identity; normal PUT owns lineup data.
        if($doc.squadName -and [string]$doc.squadName -ne [string]$state.squadName){
            Write-Log ("TEST28 IGNORE EXTRA NAME ECHO id={0}: incoming={1} keep={2}" -f `
                $SquadId,[string]$doc.squadName,[string]$state.squadName)
        }

        if($doc.formation -and -not [string]::IsNullOrWhiteSpace([string]$doc.formation)){
            $state.formation=[string]$doc.formation
        }

        if($null -ne $doc.players){
            # Native trace shows a complete 23-slot snapshot on every PUT.
            # Accept empty slots, validate non-zero IDs against the owned club,
            # and store ONLY IDs so later GETs can rebuild full ItemData.
            $newOrder=@(0)*23
            $seen=0
            $accepted=0
            $rejected=0
            foreach($slot in @($doc.players)){
                ...
                if($null -ne (Get-OwnedPlayerByItemId $itemId)){
                    ...
                    # Preserve the old card in this slot if the incoming item is
                    # not resolvable instead of silently erasing the squad.
                    $newOrder[$index]=[int]$state.order[$index]
                    $rejected++
                }
            }
            if($seen -gt 0){$state.order=@($newOrder)}
            Write-Log ("TEST28 SAVE EXTRA id={0}: slots={1} cards={2} rejected={3}" -f `
                $SquadId,$seen,$accepted,$rejected)
        }

        if($null -ne $doc.chemistry){$state.chemistry=[int]$doc.chemistry}
        if($null -ne $doc.starRating){$state.starRating=[int]$doc.starRating}
        if($null -ne $doc.rating){$state.rating=[int]$doc.rating}

        $script:Squad28Extra[[string]$SquadId]=$state
        $script:Squad28ActiveId=$SquadId

        Write-Log ("TEST30 SAVED id={0} name={1} chem={2} star={3} rating={4} formation={5}" -f `
            $SquadId,$state.squadName,$state.chemistry,$state.starRating,$state.rating,$state.formation)
        Persist-Squad30State
    }catch{
        Write-Log ("TEST28 SAVE EXTRA ERROR id={0}: {1}" -f $SquadId,$_.Exception.Message)
    }
    # TEST29: unlike the old donor save, extra-squad PUT returns its canonical
    # detail. The trace proves FIFA's own outgoing model has squadName="" for
    # extra squads, so this response is our chance to rehydrate the selected
    # screen model with the authoritative name without touching lineup state.
    return (ConvertTo-Squad28DetailJson (Build-Squad28Record $SquadId))
}

function Remove-Squad28([int] $SquadId) {
    # TEST40: FIFA's squad-delete UI needs an authoritative post-delete model,
    # not an empty object. Keep squad 1 as the permanent backing squad (FUT must
    # always have at least one squad), but allow every created extra squad to be
    # removed permanently.
    if($SquadId -le 0){return (Build-Squad28ListDocument)}
    if($SquadId -eq 1){
        Write-Log "TEST40 DELETE: refused backing squad 1 (at least one squad required)"
        return (Build-Squad28ListDocument)
    }

    $key=[string]$SquadId
    if($script:Squad28Extra.Contains($key)){
        $script:Squad28Extra.Remove($key)

        if([int]$script:Squad28ActiveId -eq $SquadId){
            $fallback=1
            $remaining=@($script:Squad28Extra.Keys | ForEach-Object {[int]$_} | Sort-Object)
            if($remaining.Count -gt 0){$fallback=[int]$remaining[0]}
            $script:Squad28ActiveId=$fallback
        }

        # A deleted squad must not remain protected by the stale-match guard.
        if([int]$script:Match33ArmedSquadId -eq $SquadId){
            $script:Match33ArmedSquadId=0
            $script:Match33ProtectedOrder=@()
            $script:Match33ProtectedCount=0
        }
        ...
        Persist-Squad30State
        Write-Log ("TEST40 DELETE squad={0}; active={1}; remainingExtras={2}" -f `
            $SquadId,$script:Squad28ActiveId,$script:Squad28Extra.Count)
    }else{
        Write-Log ("TEST40 DELETE squad={0}: already absent" -f $SquadId)
    }

    # Return the compact retail-style list so the selector redraws immediately.
    return (Build-Squad28ListDocument)
}

function Get-Test40DeleteSquadId([string]$JsonBody,[string]$Path) {
    $id=0
    if($Path -match '/squad/(\d+)(?:/delete)?$'){$id=[int]$Matches[1]}
    if($id -le 0 -and -not [string]::IsNullOrWhiteSpace($JsonBody)){
        try{
            $doc=$JsonBody|ConvertFrom-Json
            foreach($k in @('id','squadId')){
                if($doc.PSObject.Properties.Name -contains $k){
                    $candidate=0
                    if([int]::TryParse([string]$doc.$k,[ref]$candidate) -and $candidate -gt 0){$id=$candidate;break}
                }
            }
        }catch{}
    }
...
function Update-SquadFromRequest([string] $JsonBody) {
    ...
    try {
        $doc = $JsonBody | ConvertFrom-Json
        if(Test-Squad33StaleMatchWrite 1 $doc -or Test-Squad38TransientZeroReset 1 $doc){
            return (ConvertTo-Squad28DetailJson (Build-Squad28Record 1))
        }
        # TEST28: FIFA 13 can echo the previously viewed squadName in this PUT.
        # Treat normal squad saves as lineup/arithmetic saves, not rename calls.
        if ($doc.squadName -and [string]$doc.squadName -ne [string]$script:SquadName) {
            Write-Log ("TEST28 IGNORE MAIN NAME ECHO: incoming={0} keep={1}" -f [string]$doc.squadName,[string]$script:SquadName)
        }
...
function Get-StoreDocument {
    ...
    $packs = @($script:PackCatalog | Sort-Object @{Expression={[int]$_.coins}}, @{Expression={[int]$_.id}})
    # if ($isCl1217703) {
...
# 
    ...
    foreach ($pack in $packs) {
        ...
        $purchase += [ordered]@{
            ...
            sortPriority=[int]$storeRank
            # TEST34 polish aliases used by several FUT13 store widgets.
            name=[string]$pack.name
            packName=[string]$pack.name
            displayName=[string]$pack.name
            category=([string]$pack.group).ToUpperInvariant()
            level=([string]$pack.level).ToUpperInvariant()
            isPromo=(([string]$pack.group).ToLowerInvariant() -eq 'promo')
            isSpecial=(([string]$pack.group).ToLowerInvariant() -eq 'promo')
            state='active'
            visible=1
            saleType='NONE'
            purchaseCount=0
...
function Get-UserInfo {
    ...
    return ([ordered]@{
        ...
        # TEST30: expose the authoritative active squad to any header/user model
        # that does not consume /squad/<id>'s squadName directly.
        activeSquadId = [int]$script:Squad28ActiveId
        activeSquadName = [string](Get-Squad30ActiveName)
        squadName = [string](Get-Squad30ActiveName)
        ...
        activeStadium = [long]$script:Active36StadiumResourceId
        activeHomeKit = [long]$script:Active36HomeKitResourceId
        activeAwayKit = [long]$script:Active36AwayKitResourceId
        activeBadge = [long]$script:Active36BadgeResourceId
        badgeId = [int](Get-Test37ActiveAssetId ([long]$script:Active36BadgeResourceId) 0)
...
function Get-HubDocument {
    ...
    return ([ordered]@{
        ...
        coins=[int]$script:Coins
        activeSquadId=[int]$script:Squad28ActiveId
        squadCount=(1 + [int]$script:Squad28Extra.Count)
...
function Get-ClubStatsDocument {
    ...
    foreach($k in @('kits','badges','stadia','consumables','consumablesContract','consumablesTraining','consumablesTeamTalks','consumablesFitness','consumablesHealing')){
        if(-not $counts.ContainsKey($k)){$counts[$k]=0}
    }
    foreach ($item in @($script:ClubExtraItems)) {
        $row=$script:ItemByResourceId[[string][long]$item.resourceId]
        $kind=if($null -ne $row){([string]$row.kind).ToLowerInvariant()}else{([string]$item.itemType).ToLowerInvariant()}
        if($kind -eq 'player'){
            $asset = ([long]$item.resourceId -band 0xFFFFFF)
            $native = $script:NativePlayersByAsset[[string]$asset]
        ...
        }elseif($kind -eq 'kit'){$counts['kits']=[int]$counts['kits']+1}
        elseif($kind -eq 'badge'){$counts['badges']=[int]$counts['badges']+1}
        elseif($kind -eq 'stadium'){$counts['stadia']=[int]$counts['stadia']+1}
        elseif($kind -in @('contract','training','health')){
            $counts['consumables']=[int]$counts['consumables']+1
            if($kind -eq 'contract'){$counts['consumablesContract']=[int]$counts['consumablesContract']+1}
            elseif($kind -eq 'training'){$counts['consumablesTraining']=[int]$counts['consumablesTraining']+1}
            elseif($kind -eq 'health'){
                $sub=if($null -ne $row){[int]$row.cardsubtypeid}else{0}
                if($sub -in @(219,220)){$counts['consumablesFitness']=[int]$counts['consumablesFitness']+1}
                elseif($sub -in @(221,222)){$counts['consumablesTeamTalks']=[int]$counts['consumablesTeamTalks']+1}
                else{$counts['consumablesHealing']=[int]$counts['consumablesHealing']+1}
            }
        }
    }
...
function Get-Body([string] $Path, [string] $Method = 'GET', [string] $RequestBody = '') {
    ...
    if ($clean -match '/user/accountinfo$') {
        $badgeAsset=Get-Test37ActiveAssetId ([long]$script:Active36BadgeResourceId) 0
        return ([ordered]@{userAccountInfo=[ordered]@{personas=@([ordered]@{
            personaId=[long]$script:PersonaId;personaName='FUT13 Local';isReturningUser=$true;onlineAccess=$true;trial=$false;userState=$null
            userClubList=@([ordered]@{year='2013';assetId=1;teamId=1;lastAccessTime=1;platform='pc';clubName='FUT13 Local';clubAbbr='LFT';established=2012;divisionOnline=10;badgeId=$badgeAsset;skuAccessList=[ordered]@{FFA13PC=1}})
            trialFree=$false
        })}}|ConvertTo-Json -Depth 8 -Compress)
    }
    if ($clean -match '/settings$') {
        ...
        return '{"configs":[{"type":"maximumTradePileSize","value":9999},{"type":"getOperationTimeoutSec","value":30},{"type":"clubCreateThreshold","value":0},{"type":"fifaPointsCancelTransactionFix","value":1},{"type":"firstPartyCommerceFlowVersion","value":1}]}'
    }
    if ($clean -eq '/ut/delete/auth') { return '{}' }
    # TEST34 cosmetic/optional frontend probes. These are deliberately narrow:
    # keep parser-critical routes untouched while preventing harmless UI probes
    # from turning into noisy 404s.
    if ($clean -match '/eventfeed$' -and $Method -eq 'GET') {
        return '{"event":[],"events":[]}'
    }
    if ($clean -match '/clientdata/(tutorialpopups|managerquest)$' -and $Method -eq 'GET') {
        return '{}'
    }
    if ($clean -match '/clientdata/pileSize$' -and $Method -eq 'GET') {
        return ([ordered]@{
            entries=@(
                [ordered]@{key=2;value=9999},
                [ordered]@{key=3;value=9999},
                [ordered]@{key=4;value=9999}
            )
        }|ConvertTo-Json -Depth 5 -Compress)
    }
    if ($clean -match '/match/reset$') {
        $script:Match33ArmedSquadId=0;$script:Match33ProtectedOrder=@();$script:Match33ProtectedCount=0
        return '{}'
    }
    ...
    if ($clean -match '/match(?:/1)?$' -and $Method -in @('POST','PUT')) {
        $matchSquadId=[int]$script:Squad28ActiveId
        try{
            if(-not [string]::IsNullOrWhiteSpace($RequestBody)){
                $matchRequest=$RequestBody|ConvertFrom-Json
                if($null -ne $matchRequest.squadId -and [int]$matchRequest.squadId -gt 0){$matchSquadId=[int]$matchRequest.squadId}
            }
        }catch{}
        Arm-Match33Squad $matchSquadId
        $matchStadium=Get-Test37ActiveAssetId ([long]$script:Active36StadiumResourceId) 34
        $matchHomeKit=Get-Test37ActiveAssetId ([long]$script:Active36HomeKitResourceId) 14
        $matchAwayKit=Get-Test37ActiveAssetId ([long]$script:Active36AwayKitResourceId) 15
        return ([ordered]@{
            matchId=1;userTeamId=1;opponentTeamId=241;matchDifficulty=3;matchLengthMin=6
            stadiumId=$matchStadium;homeKit=$matchHomeKit;awayKit=$matchAwayKit;squadId=$matchSquadId
...
# Keep the proven bootstrap envelopes. Transaction is the one exception:
    ...
    # TEST36: S11 proves stadium/kit activation uses PUT /item/<ownedItemId>
    # with {"itemState":"active"}. Handle that separately from consumables.
    if ($clean -match '/game/fifa13/item/(\d+)$' -and $Method -eq 'PUT') {
        return (Set-ActiveClubItem36 ([int64]$Matches[1]) $RequestBody)
    }
    # TEST32: native consumable application route measured in S9.
    # Example: POST /ut/game/fifa13/item/25897 {"apply":[{"id":22641}]}
    if ($clean -match '/game/fifa13/item/(\d+)$' -and $Method -eq 'POST') {
        return (Apply-ConsumableDocument32 ([int64]$Matches[1]) $RequestBody)
    }
    ...
    if ($clean -match '/squad/active$') {
        if($Method -in @('POST','PUT')){
            try{
                $doc=$RequestBody | ConvertFrom-Json
                $candidate=0
                if($null -ne $doc.id){$candidate=[int]$doc.id}
                elseif($null -ne $doc.squadId){$candidate=[int]$doc.squadId}
                if($candidate -gt 0){[void](Set-Squad28Active $candidate)}
            }catch{}
        }
        return (ConvertTo-Squad28DetailJson (Build-Squad28Record ([int]$script:Squad28ActiveId)))
    }
    # TEST41: exact FIFA 13 retail delete transport observed in S15.
    # The client does NOT send HTTP DELETE. It sends:
    #   GET /ut/delete/game/fifa13/squad/<id>
    # Depending on URL normalization, $clean may retain or drop the leading /ut.
    if($Method -eq 'GET' -and
       ($clean -match '(?:^|/)delete/game/fifa13/squad/(\d+)$')){
        $deleteSquadId=[int]$Matches[1]
        Write-Log ("TEST41 FIFA13 DELETE ROUTE squad={0} path={1}" -f $deleteSquadId,$clean)
        return (Remove-Squad28 $deleteSquadId)
    }

    # TEST40: accept the common retail delete shapes. The numbered DELETE route
    # remains supported, plus body-based DELETE /squad and POST/DELETE /squad/N/delete.
    if(($clean -match '/squad$' -and $Method -eq 'DELETE') -or
       ($clean -match '/squad/\d+/delete$' -and $Method -in @('POST','DELETE'))){
        $deleteId=Get-Test40DeleteSquadId $RequestBody $clean
        return (Remove-Squad28 $deleteId)
    }
    ...
    if ($clean -match '/squad/(\d+)$') {
        ...
        if($Method -eq 'DELETE'){
            return (Remove-Squad28 $requestedSquadId)
        }

        if($Method -in @('POST','PUT')){
            if($requestedSquadId -eq 1){
                $script:Squad28ActiveId=1
                [void](Update-SquadFromRequest $RequestBody)
                Persist-Squad30State
                return (ConvertTo-Squad31SelectedDetailJson (Build-Squad28Record 1))
            }
            [void](Save-Squad28Extra $requestedSquadId $RequestBody)
            return (ConvertTo-Squad31SelectedDetailJson (Build-Squad28Record $requestedSquadId))
        }

        if($Method -eq 'GET'){
            # Native JSONL proves numbered GET is the actual selection signal.
            [void](Set-Squad28Active $requestedSquadId)
            return (ConvertTo-Squad31SelectedDetailJson (Build-Squad28Record $requestedSquadId))
        }
        return '{}'
    }
    if ($clean -match '/squad$') {
        if($Method -eq 'POST'){
            try{
                $doc=$RequestBody | ConvertFrom-Json
                if($null -ne $doc.id -and [int]$doc.id -eq 0){
                    return (New-Squad28 $RequestBody)
                }
                if($null -ne $doc.id -and [int]$doc.id -gt 1){
                    return (Save-Squad28Extra ([int]$doc.id) $RequestBody)
                }
            }catch{}
        ...
        if($Method -eq 'PUT'){
            try{
                ...
                if($null -ne $doc.id -and [int]$doc.id -gt 1){
                    $sid=[int]$doc.id
                    [void](Save-Squad28Extra $sid $RequestBody)
                    return (ConvertTo-Squad31SelectedDetailJson (Build-Squad28Record $sid))
                }
            }catch{}
            [void](Update-SquadFromRequest $RequestBody)
            Persist-Squad30State
        ...
        return (ConvertTo-Squad31SelectedDetailJson (Build-Squad28Record ([int]$script:Squad28ActiveId)))
    }
    ...
    # TEST38: S13 exposed that this later route was overriding TEST37 and still
    # forcing the old 30/100/30 limits. Keep the exact proven entries[] contract,
    # but raise every capacity bucket to a practical maximum.
...
function Get-ClubStatsDocument{
    ...
    foreach($it in @($script:ClubExtraItems)){
        ...
        elseif($kind -in @('training','contract','health','coins','pack')){
            $counts.consumables++
            if($kind -eq 'contract'){$counts.consumablesContract++}
            elseif($kind -eq 'training'){$counts.consumablesTraining++}
            elseif($kind -eq 'health'){
                $row=$script:ItemByResourceId[[string][long]$it.resourceId]
                $sub=if($null -ne $row -and $null -ne $row.cardsubtypeid){[int]$row.cardsubtypeid}else{0}
                if($sub -in @(219,220)){$counts.consumablesFitness++}
                elseif($sub -in @(221,222)){$counts.consumablesTeamTalks++}
                else{$counts.consumablesHealing++}
            }
        }
    }
...
function New-PackPurchaseDocument([string]$RequestBody='', [string]$Path='') {
    ...
    try {
        ...
        $hasExplicitPlayerCount = ($pack.PSObject.Properties.Name -contains 'playerCount')
        $playerOnly = (([int]$pack.id -ge 7) -and -not $hasExplicitPlayerCount)
        $playerCount = if ($hasExplicitPlayerCount) {
            [Math]::Max(0,[Math]::Min($size,[int]$pack.playerCount))
        } elseif ($playerOnly) {
            $size
        } else {
            [Math]::Min($size,(Get-WeightedPlayerCount))
        }
        $slotKinds = @()
...
# ---------------------------------------------------------------------------
# TEST32: consumable application
# Native trace S9 proves FIFA 13 applies a consumable using:
# POST /ut/game/fifa13/item/<consumableItemId>
# {"apply":[{"id":<playerItemId>}, ...]}
#
# The old server had no handler for this route.  Resolve the owned consumable
# from its native resourceId, update the addressed owned player cards, consume
# the card exactly once, persist the mutated card state, and return the updated
# ItemData so the retail client can redraw immediately.
# ---------------------------------------------------------------------------
function Remove-OwnedItem32([long]$ItemId) {
    $script:PurchasedItems = @($script:PurchasedItems | Where-Object { [int64]$_.id -ne $ItemId })
    $script:PurchasedDuplicatePairs = @($script:PurchasedDuplicatePairs | Where-Object { [int64]$_.itemId -ne $ItemId })
    $script:ClubExtraItems = @($script:ClubExtraItems | Where-Object { [int64]$_.id -ne $ItemId })
    $script:TradePileItems = @($script:TradePileItems | Where-Object { [int64]$_.id -ne $ItemId })
}

function Get-PlayerRating32([object]$Player) {
    if ($null -ne $Player -and $Player.PSObject.Properties.Name -contains 'rating') {
        $r = [int]$Player.rating
        if ($r -gt 0) { return $r }
    }
    if ($null -ne $Player -and $Player.PSObject.Properties.Name -contains 'resourceId') {
        $facts = Get-ResourceFacts ([long]$Player.resourceId)
        if ($null -ne $facts -and [int]$facts.rating -gt 0) { return [int]$facts.rating }
    }
    return 75
}

function Set-PlayerScalar32([object]$Player,[string]$Name,[int]$Value) {
    if ($null -eq $Player) { return }
    $Player | Add-Member -NotePropertyName $Name -NotePropertyValue $Value -Force
}

function Get-ContractAmount32([object]$Row,[object]$Player) {
    $rating = Get-PlayerRating32 $Player
    $field = if ($rating -lt 65) { 'bronze' } elseif ($rating -lt 75) { 'silver' } else { 'gold' }
    $amount = 0
    if ($null -ne $Row -and $Row.PSObject.Properties.Name -contains $field) {
...
function Apply-Training32([object]$Player,[object]$Row) {
    $sub = if ($null -ne $Row.cardsubtypeid) { [int]$Row.cardsubtypeid } else { 0 }
    $amount = if ($null -ne $Row.amount) { [int]$Row.amount } else { 0 }
    if ($amount -le 0) { return }

    # FIFA 13 player-training subtypes:
    # 51-56 = outfield attribute cards, 57 = all-outfield,
    # 61-66 = goalkeeper attributes, 67 = all-GK.
    $map = @{
        51='pace'; 52='shooting'; 53='passing'; 54='dribbling'; 55='defending'; 56='heading'
        61='gkDiving'; 62='gkHandling'; 63='gkKicking'; 64='gkPositioning'; 65='gkReflexes'; 66='gkOneOnOne'
    }

    $boost = [ordered]@{}
    if ($map.ContainsKey($sub)) {
        $boost[[string]$map[$sub]] = $amount
    } elseif ($sub -eq 57) {
        foreach ($k in @('pace','shooting','passing','dribbling','defending','heading')) { $boost[$k] = $amount }
    } elseif ($sub -eq 67) {
        foreach ($k in @('gkDiving','gkHandling','gkKicking','gkPositioning','gkReflexes','gkOneOnOne')) { $boost[$k] = $amount }
    } else {
        # Position/formation/modifier cards live in the higher subtype range.
        # Do not corrupt player identity by guessing those structures.
        return
    }

    # Persist both named boosts and a compact trainingBoost object.  Named
    # fields are harmless to old parsers and allow the retail detail model to
    # consume whichever representation it recognises.
    $training = [ordered]@{}
    if ($Player.PSObject.Properties.Name -contains 'trainingBoost' -and $null -ne $Player.trainingBoost) {
        foreach ($prop in $Player.trainingBoost.PSObject.Properties) {
            $training[[string]$prop.Name] = [int]$prop.Value
        }
    }
    foreach ($k in $boost.Keys) {
        $old = if ($training.Contains($k)) { [int]$training[$k] } else { 0 }
        $training[$k] = [Math]::Min(99,$old + [int]$boost[$k])
        $Player | Add-Member -NotePropertyName ([string]$k) -NotePropertyValue ([int]$training[$k]) -Force
    }
    $Player | Add-Member -NotePropertyName trainingBoost -NotePropertyValue ([pscustomobject]$training) -Force
}

function Apply-ConsumableDocument32([long]$ConsumableItemId,[string]$JsonBody) {
    $source = Get-OwnedPlayerByItemId $ConsumableItemId
    if ($null -eq $source) {
        Write-Log ("TEST32 CONSUMABLE: source item {0} not owned" -f $ConsumableItemId)
        return '{"itemData":[],"success":false,"reason":"item not found"}'
    }

    $row = $null
    if ($source.PSObject.Properties.Name -contains 'resourceId') {
        $row = $script:ItemByResourceId[[string][long]$source.resourceId]
    }
    if ($null -eq $row) {
        # TEST39 quick compatibility for FIFA13 formation modifier cards. The
        # catalogue builder intentionally skipped unresolved formation subtypes
        # 83/133, so old/owned modifier items can exist without a lookup row.
        $sourceType=([string]$source.itemType).ToLowerInvariant()
        $sourceSub=if($null -ne $source.cardsubtypeid){[int]$source.cardsubtypeid}else{0}
        if($sourceType -eq 'training' -and $sourceSub -in @(83,133)){
            $row=[pscustomobject]@{kind='formation';cardsubtypeid=$sourceSub;formation='';amount=0}
        }else{
            Write-Log ("TEST32 CONSUMABLE: no catalog row for item={0} resource={1}" -f $ConsumableItemId,$source.resourceId)
            return '{"itemData":[],"success":false,"reason":"unsupported item"}'
        }
    }

    $kind = ([string]$row.kind).ToLowerInvariant()
    if ($kind -notin @('contract','health','training','formation')) {
        Write-Log ("TEST32 CONSUMABLE: unsupported kind={0} item={1}" -f $kind,$ConsumableItemId)
        return '{"itemData":[],"success":false,"reason":"unsupported consumable"}'
    }

    try { $doc = $JsonBody | ConvertFrom-Json } catch { $doc = $null }
    $targetIds = @()
    if ($null -ne $doc) {
        foreach ($entry in @($doc.apply)) {
            if ($null -ne $entry.id) {
                $id = [int64]$entry.id
                if ($id -gt 0 -and $id -notin $targetIds) { $targetIds += $id }
            }
        }
    }
    if ($targetIds.Count -eq 0) {
        return '{"itemData":[],"success":false,"reason":"no targets"}'
    }

    $updated = @()
    foreach ($targetId in $targetIds) {
        $player = Get-OwnedPlayerByItemId $targetId
        if ($null -eq $player -or ([string]$player.itemType).ToLowerInvariant() -ne 'player') {
            Write-Log ("TEST32 CONSUMABLE: target {0} is not an owned player" -f $targetId)
            continue
        }

        if ($kind -eq 'contract') {
            $amount = Get-ContractAmount32 $row $player
            $old = if ($null -ne $player.contract) { [int]$player.contract } else { 0 }
            Set-PlayerScalar32 $player 'contract' ([Math]::Min(99,$old + $amount))
            Write-Log ("TEST32 CONTRACT: consumable={0} player={1} +{2} => {3}" -f $ConsumableItemId,$targetId,$amount,$player.contract)
        }
        elseif ($kind -eq 'health') {
            $sub = if ($null -ne $row.cardsubtypeid) { [int]$row.cardsubtypeid } else { 0 }
            $amount = if ($null -ne $row.amount) { [int]$row.amount } else { 0 }
            ...
            if ($sub -in @(219,220)) {
                # Fitness consumables.
                $old = if ($null -ne $player.fitness) { [int]$player.fitness } else { 0 }
                Set-PlayerScalar32 $player 'fitness' ([Math]::Min(99,$old + [Math]::Max(1,$amount)))
                Write-Log ("TEST32 FITNESS: consumable={0} player={1} +{2} => {3}" -f $ConsumableItemId,$targetId,$amount,$player.fitness)
            }
            elseif ($sub -in @(221,222)) {
                # Morale/team-talk consumables.
                $old = if ($null -ne $player.morale) { [int]$player.morale } else { 0 }
                Set-PlayerScalar32 $player 'morale' ([Math]::Min(100,$old + [Math]::Max(1,$amount)))
                Write-Log ("TEST32 MORALE: consumable={0} player={1} +{2} => {3}" -f $ConsumableItemId,$targetId,$amount,$player.morale)
            }
            else {
                # Injury/healing consumables (body-part specific subtypes 211-218).
                $old = if ($null -ne $player.injuryGames) { [int]$player.injuryGames } else { 0 }
                $new = [Math]::Max(0,$old - [Math]::Max(1,$amount))
                Set-PlayerScalar32 $player 'injuryGames' $new
                if ($new -eq 0) { Set-PlayerScalar32 $player 'injuryType' 0 }
                Write-Log ("TEST32 HEALING: consumable={0} player={1} -{2} injuryGames => {3}" -f $ConsumableItemId,$targetId,$amount,$new)
            }
        }
        elseif ($kind -eq 'training') {
            $sub=if($null -ne $row.cardsubtypeid){[int]$row.cardsubtypeid}else{0}
            if($sub -gt 67){
                # Higher FIFA13 training subtypes are modifier cards. For the
                # quick formation path, align the player with the active squad.
                $formation=[string](Get-Squad30ActiveFormation)
                if([string]::IsNullOrWhiteSpace($formation)){$formation='f442'}
                Set-PlayerScalar32 $player 'formation' $formation
                Write-Log ("TEST39 FORMATION MODIFIER: consumable={0} subtype={1} player={2} => {3}" -f $ConsumableItemId,$sub,$targetId,$formation)
            }else{
                Apply-Training32 $player $row
                Write-Log ("TEST32 TRAINING: consumable={0} subtype={1} player={2}" -f $ConsumableItemId,$row.cardsubtypeid,$targetId)
            }
        }
        elseif ($kind -eq 'formation') {
            $formation=[string](Get-Squad30ActiveFormation)
            if([string]::IsNullOrWhiteSpace($formation)){$formation='f442'}
            Set-PlayerScalar32 $player 'formation' $formation
            Write-Log ("TEST39 FORMATION MODIFIER FALLBACK: consumable={0} player={1} => {2}" -f $ConsumableItemId,$targetId,$formation)
        }
    ...
    if ($updated.Count -eq 0) {
        return '{"itemData":[],"success":false,"reason":"no valid player targets"}'
    }

    # One consumable card is spent once even when its retail body addresses
    # multiple players (as observed in S9).
    Remove-OwnedItem32 $ConsumableItemId
    Persist-SaveState
    Persist-Squad30State

    # Deliberately expose several retail-safe aliases.  itemData is the main
    # parser contract; items/applied are useful to screen models that consume
    # the mutation response directly.
    $response = [ordered]@{
        itemData = @($updated)
        items = @($updated)
        applied = @($updated | ForEach-Object { [ordered]@{id=[int64]$_.id;success=$true} })
        consumedItemId = [int64]$ConsumableItemId
        success = $true
    }
